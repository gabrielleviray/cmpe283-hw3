...
// #                                                      # 
// ########################################################
atomic64_t exit_counter_value = ATOMIC64_INIT(0);
EXPORT_SYMBOL(exit_counter_value);

atomic64_t exit_duration_value = ATOMIC64_INIT(0);
EXPORT_SYMBOL(exit_duration_value);
...
// #                                                      # 
// ########################################################
atomic64_t exit_numbers[69] = { [0...68] = ATOMIC_INIT(0)};
EXPORT_SYMBOL(exit_numbers);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	// #                                                     # 
	// #######################################################
	else if ( eax == 0x4FFFFFFFE){
		if ( ecx == 65 || ecx == 42 || ecx == 38 || ecx ==  35 || ecx < 0 || ecx > 68 ) { // Undefined in SDM
			printk(KERN_INFO "Exit type: (%u) is undefined in SDM", ecx);
			eax = 0;
			ebx = 0;
			ecx = 0;
			edx = 0xFFFFFFFF;
			...
			printk(KERN_INFO "CPUID(0x4FFFFFFE): Exit number: (%u) Number of exits:(%d) \n", ecx, arch_atomic_read(&exit_numbers[ecx]));

			// exit type is not enabled in KVM
			eax = arch_atomic_read(&exit_numbers[ecx]);  
			ebx = 0;
			ecx = 0;
			edx = 0;
		} else {
